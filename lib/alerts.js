...
class AlertManager {
  ...
  async evaluate(state, now = Date.now()) {
    ...
    for (const [machine, m] of Object.entries(state.machines ?? {})) {
      ...
      // Sweep deleted guests: any guest with state but no longer in m.guests is gone (only when host is up,
      // so we don't confuse a scrape failure with a deletion). Resolves stuck alerts and forgets the guest.
      if (m.status === 'up') {
        const present = new Set((m.guests ?? []).map((g) => g.name));
        const tracked = new Set();
        for (const key of this.#state.keys()) {
          if (!key.startsWith(`${machine}/`)) continue;
          const guestName = key.slice(machine.length + 1).split('/')[0];
          if (guestName === '_host') continue;
          if (!present.has(guestName)) tracked.add(guestName);
        }
        for (const lr of this.#lastSeenRunning) {
          if (!lr.startsWith(`${machine}/`)) continue;
          const guestName = lr.slice(machine.length + 1);
          if (!present.has(guestName)) tracked.add(guestName);
        }
        for (const guestName of tracked) {
          for (const t of this.#muteSweep(machine, guestName, now)) push(t);
          this.#lastSeenRunning.delete(`${machine}/${guestName}`);
          for (const key of [...this.#state.keys()]) {
            if (key.startsWith(`${machine}/${guestName}/`)) this.#state.delete(key);
          }
        }
      }
    }
  ...
  // Force-resolve any firing alerts and clear pending state for a machine/guest.
  // Used when a target becomes muted or is deleted from inventory.
